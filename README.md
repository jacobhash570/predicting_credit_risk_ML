...
### Prediction
I belive Logistic Regression is more appropriate for this dataset. We are working with numeric data here which the model handles well. However, if we have outliers the Random Forest Classifier will be of more value. 
 
### Outcome 
The Random Forest Classifier model preformed much better with a score of .64 compared to the Logistic Regression mdoel with a score of .51. However, the gap between the training and test is smaller in the Logistic Regression model, so when it is scaled it should preform better. My resaction to the outcome is that we have outliers that the first model considered.

### Scaled 
After scaling, the Logistic Regression preformed the best. Confirming suspicion that the data contained outliers.
